...
struct SEG
{
    ...
    ll number(ll l, ll r, ll val, ll x, ll lx, ll rx)
    {
        if (lx > r or rx < l)
            return 0;
        if (lx >= l and rx <= r)
        ...
        ll mid = lx + (rx - lx) / 2;
        return number(l, r, val, 2 * x + 1, lx, mid) + number(l, r, val, 2 * x + 2, mid + 1, rx);
    }
    ll number(ll l, ll r, ll val)
    {
        return number(l, r, val, 0, 0, siz - 1);
    }
};
class Solution
{
public:
    long long numberOfPairs(vector<int> &a, vector<int> &b, int diff)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            pairs += obj.number(i + 1, n - 1, v[i] - diff);
        }
        return pairs;
    }
};